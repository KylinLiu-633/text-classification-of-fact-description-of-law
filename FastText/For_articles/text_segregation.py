...
if __name__ == "__main__":
    ...
    with open(text_file, "r", encoding='utf-8') as readfile:
        while True:
            ...
            if tmp1 == "":
                tmp1 = text_1
            text_part_1.append(tmp1)
            if tmp2 == "":
                text_part_2.append(text_1)
            else:
                text_part_2.append(tmp2)
            tmp3 = tmp1 + tmp2
            if tmp3 == "":
                tmp3 = text_1
            text_part_3.append(tmp3)
            if text_6 != "":
                tmp4 = text_6.replace(text_8, "")
            if tmp4 == "":
                tmp4 = text_1
            text_part_4.append(tmp4)

            cnt += 1

        print(cnt)

    with open("data/train_seg_1.txt", "w", encoding='utf-8') as file1:
        for index in text_part_1:
            file1.write(index + "\n")
    with open("data/train_seg_2.txt", "w", encoding='utf-8') as file2:
        for index in text_part_2:
            file2.write(index + "\n")
    with open("data/train_seg_3.txt", "w", encoding='utf-8') as file3:
        for index in text_part_3:
            file3.write(index + "\n")
    with open("data/train_seg_4.txt", "w", encoding='utf-8') as file3:
        for index in text_part_4:
            file3.write(index + "\n")
    with open("data/label.txt", "w", encoding='utf-8') as file3:
        for index in label:
            file3.write(index + "\n")
